...
class ConstructorStandingExtractor implements ExtractorInterface
{
    public function create_table($xml, string $tableName): string
    {
        ...
        foreach ($xml->StandingsTable->StandingsList as $standingList) {
            ...
            foreach ($standingList->ConstructorStanding as $constructorStanding) {
                foreach ($constructorStanding->attributes() as $attribute => $value) {
                    $attributes .= QueryBuilder::build_attributes_columnlist($attribute);
                }

                foreach ($constructorStanding->Constructor->attributes() as $attribute => $value) {
